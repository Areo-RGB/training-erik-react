...
export default function Kettenrechner() {
  ...
  return (
    <div className={`w-full h-full min-h-[60vh] flex flex-col items-center justify-center relative px-4 ${status !== 'config' ? 'fixed inset-0 z-40 bg-[#0B0E14]' : ''}`}>
      ...
      <div className={`w-full max-w-2xl bg-[#151A23] rounded-2xl sm:rounded-3xl border border-white/5 shadow-xl flex flex-col items-center relative overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${status === 'config' ? 'p-6 sm:p-12 min-h-[400px] sm:min-h-[500px] animate-enter-scale' : 'h-full w-full max-w-none rounded-none p-4 sm:p-12 bg-[#0B0E14] border-none'}`}>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3 w-full mb-4 sm:mb-6">
                {[1,2,3,4,5,6,7,8,9].map((digit, i) => (
                  <button
                    ...
                    className="animate-enter opacity-0 aspect-square p-3 sm:p-4 text-xl sm:text-2xl font-bold bg-[#1E2532] text-[#F1F5F9] border border-white/5 rounded-xl hover:bg-[#2A3441] hover:-translate-y-0.5 active:translate-y-0 transition-all touch-manipulation"
                  >
                    {digit}
                  </button>
                ))}
                <button onClick={() => appendDigit(0)} className="animate-enter delay-300 opacity-0 aspect-square p-3 sm:p-4 text-xl sm:text-2xl font-bold bg-[#1E2532] text-[#F1F5F9] border border-white/5 rounded-xl hover:bg-[#2A3441] hover:-translate-y-0.5 active:translate-y-0 transition-all touch-manipulation">0</button>
                <button onClick={() => setUserAnswer('')} className="animate-enter delay-300 opacity-0 aspect-square p-3 sm:p-4 text-lg sm:text-xl font-bold bg-red-900/20 text-red-400 border border-red-500/20 rounded-xl hover:bg-red-900/40 hover:-translate-y-0.5 active:translate-y-0 transition-all touch-manipulation">C</button>
              </div>
